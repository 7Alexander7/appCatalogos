modificarTitulo('Productos / Nuevo Producto')
var imagen = null
var nombreImagen = ""
...
document.getElementById('uploadButton').addEventListener('click', function (event) {
  ...
  .then(json => {
    if (json.success == true) {
      ...
      notificaciones("PRODUCTOS", "se ha guardado la imagen exitosamente", "bien")
    } else {
      notificaciones("PRODUCTOS", "ha existido un error vuelva a intentarlo", "mal")
    }
  })
  .catch(error => {
...
function guardar(e) {
  ...
  if (idEmpresa != undefined && idEmpresa != null && idEmpresa != "") {
    if (nombreImagen != "" && imagen != null) {
      ...
      fetch(ipServidor + 'api/productos/nuevo', {
        headers: {
        ...
        .then(json => {
          if (json.success == true) {
            notificaciones("PRODUCTOS", "los datos se han almacenado correctamente", "bien")
          } else {
            notificaciones("PRODUCTOS", "ha existido un error vuelva a intentarlo", "mal")
          }
          limpiarInputs()
      ...
      notificaciones("PRODUCTOS", "debe subir primero una imagen del servicio", "mal")
    }
  } else {
    notificaciones("PRODUCTOS", "ha ocurrido un error", "mal")
  }
}
